...
If you have seed-values that should only be inserted, but not updated when the seed-file is run again, you can use seed_once:

    User.seed_once(:login) do |s|
      s.login = "admin"
      s.email = "admin@example.com"
    end

This behaves as you would expect: a user with the login "admin" is created the first time this seed-file is run. Running the seed-file again will not update the existing "admin" user.
...
syntax over the seen_many() syntax.  You can even use a 
SeedFu::Writer::SeedOnce writer if you prefer already seeded values
not to be overwritten. Easy-peasy.
